...
class RepositoryUseCase(private val remote: RemoteDataSource): RepositoryInterface {
    private val result = PublishSubject.create<UserDataEntity>()
    private val mCompositeDisposable = CompositeDisposable()

    @SuppressLint("CheckResult")
    override fun getResult(content: String): Flowable<UserData> {
        remote.getResult(content)
        ...
        return value.map { ClassMapper.mapEntityToDomain(it) }
    }
}